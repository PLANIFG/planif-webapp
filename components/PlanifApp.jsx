...
async function askClaudeOnce(promptText, maxTokens) {
  ...
  if (!response.ok) {
    throw new Error(`réseau/API (${response.status})${data?.error ? " — " + data.error : ""}`);
  }
...
// from the model), but not on network/API errors — those fail immediately.
...
async function askClaude(promptText, maxTokens = 3000) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      lastErr = e;
      if (String(e.message).startsWith("réseau/API")) throw e;
    }
  }
  throw lastErr;
}
...
export default function App() {
  ...
  const generateTransition = async () => {
    ...
    try {
      ...
      setTransitionError(`Échec de la génération : ${e.message || "erreur inconnue"}`);
    } finally {
  ...
  const generateBatch = async () => {
    ...
    try {
      ...
      setError(`La génération a échoué : ${e.message || "erreur inconnue"}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const regenerateOne = async (id) => {
    ...
    try {
      ...
      setError(`La régénération a échoué : ${e.message || "erreur inconnue"}`);
    } finally {
