...
function TextField({ value, onChange, placeholder, className = "" }) {
  return (
    <input
      ...
      className={`w-full bg-white border border-[#DCD3C2] rounded-lg px-3 py-2 text-[15px] text-[#2B2A26] placeholder-[#B3A990] focus:outline-none focus:ring-2 focus:ring-[#0E79B2] focus:border-transparent ${className}`}
    />
  );
}
  ...
  return (
    <button
      ...
      className={`px-3 py-1.5 rounded-full text-sm font-semibold border transition-colors ${
        active ? "border-transparent" : "text-[#7A7362] border-[#DCD3C2] bg-white hover:border-[#0E79B2]"
...
function IconBtn({ onClick, title, children, danger }) {
  return (
    <button onClick={onClick} title={title} className={`inline-flex items-center justify-center w-8 h-8 rounded-lg transition-colors ${danger ? "hover:bg-red-50 text-red-500" : "hover:bg-[#E4EEE4] text-[#0E79B2]"}`}>
      {children}
    </button>
  );
}
  ...
  const generateBatch = async () => {
    ...
    try {
      ...
      for (let i = 0; i < effectiveCount; i++) {
        try {
          const raw = await askClaude(buildSinglePrompt({ theme, ages, lieux, avoidNames: names, isMercredi }));
              ...
              <>
                ...
                <div className="absolute right-0 top-12 z-20 bg-white border border-[#E3DACB] rounded-xl shadow-lg py-1.5 w-56">
                  <button
                    ...
                    className="w-full text-left px-4 py-2.5 text-sm font-semibold text-[#2B2A26] hover:bg-[#E8E9F3]"
                  >
                    Ma bibliothèque
                  </button>
                  <button
                    ...
                    className="w-full text-left px-4 py-2.5 text-sm font-semibold text-[#2B2A26] hover:bg-[#E8E9F3] disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className="w-full text-left px-4 py-2.5 text-sm font-semibold text-[#7A7362] hover:bg-[#E8E9F3]"
                  >
                    Se déconnecter
                  </button>
                </div>
            ...
            editingLieu === l ? (
              <div key={l} className="flex items-center gap-1">
                <input
                  ...
                  className="px-3 py-1.5 rounded-full text-sm font-semibold border border-[#0E79B2] bg-white w-32"
                />
              </div>
            ) : (
          ...
          <div className="mb-5 mt-4">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2">
              <select
                ...
                className="bg-white border border-[#DCD3C2] rounded-lg px-3 py-2 text-[15px] text-[#2B2A26] focus:outline-none focus:ring-2 focus:ring-[#0E79B2]"
              >
              ...
              <input
                ...
                className="bg-white border border-[#DCD3C2] rounded-lg px-3 py-2 text-[15px] text-[#2B2A26] w-24 focus:outline-none focus:ring-2 focus:ring-[#0E79B2]"
              />
            </div>
      ...
      {kept.length > 0 && (
        <SectionCard>
          <h2 className="text-lg font-bold mb-3 flex items-center gap-2" style={{ fontFamily: "Baloo 2, sans-serif", color: COLORS.mossDark }}>
            <Check size={18} className="text-[#0E79B2]" /> Retenues pour la journée ({kept.length})
          </h2>
      ...
      <SectionCard className="!bg-[#F3C9BE] !border-[#F3C9BE]">
        <label className="flex items-center gap-2 cursor-pointer w-fit">
          <input
            ...
            className="w-4 h-4 rounded accent-[#0E79B2]"
          />
          ...
          <div className="mt-3 ml-6">
            ...
            {transitionData && !transitionError && (
              <div className="mt-3 p-3 rounded-lg border border-[#E3DACB] bg-white">
                <p className="text-xs font-bold text-[#0E79B2] mb-2">✓ Prêtes — s'ajouteront à l'aperçu</p>
                  ...
                  <div className="pt-2 border-t border-[#EDE6D8]">
                    ...
                    {transitionData.imagePrompts.map((p, i) => (
                      <div key={i} className="flex items-center gap-2 bg-[#FBF3E4] rounded px-2 py-1 mt-1">
                        ...
                        <button onClick={() => navigator.clipboard.writeText(p)} className="text-[10px] font-bold text-[#0E79B2] bg-white border border-[#DCD3C2] rounded px-2 py-1 shrink-0">Copier</button>
                      </div>
                    ))}
                    ...
                    <div className="mt-2">
                      <label className="text-xs font-semibold text-[#0E79B2] cursor-pointer inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#DCD3C2] hover:border-[#0E79B2]">
  ...
  return (
    <div className="bg-white border border-[#E3DACB] rounded-2xl p-4 flex flex-col" style={{ boxShadow: "0 1px 2px rgba(43,42,38,0.04)" }}>
      <div className="flex items-start justify-between gap-2 mb-1">
        ...
        <button onClick={onEdit} className="shrink-0 text-[#B3A990] hover:text-[#0E79B2]" title="Modifier"><Pencil size={15} /></button>
      </div>
      ...
      <div className="mb-3">
        ...
        {idea.amorce ? (
          isEditing ? (
            <textarea value={idea.amorce} onChange={(e) => onUpdate({ amorce: e.target.value })} rows={3} className="w-full bg-white border border-[#DCD3C2] rounded-lg px-2.5 py-1.5 text-sm text-[#2B2A26] focus:outline-none focus:ring-2 focus:ring-[#0E79B2]" />
          ) : (
      ...
      <div className="mb-3">
        ...
        {isEditing && <button onClick={() => onAddListItem("deroulement")} className="mt-1 text-xs font-semibold text-[#0E79B2] flex items-center gap-1"><Plus size={12} /> Étape</button>}
      </div>
      ...
      <div className="mb-4">
        ...
        {isEditing && <button onClick={() => onAddListItem("materiel")} className="mt-1 text-xs font-semibold text-[#0E79B2] flex items-center gap-1"><Plus size={12} /> Item</button>}
      </div>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <SectionCard>
        ...
        <button onClick={addGroup} className="mt-3 flex items-center gap-1.5 text-sm font-semibold text-[#0E79B2] hover:underline"><Plus size={15} /> Ajouter un groupe</button>
      </SectionCard>
      ...
      <SectionCard>
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          <button onClick={scheduleOps.addFixe} className="flex items-center gap-1.5 text-sm font-semibold text-[#0E79B2] border border-[#DCD3C2] rounded-lg px-3 py-1.5 hover:border-[#0E79B2]">
            <Plus size={14} /> Bloc fixe
          </button>
          <button onClick={scheduleOps.addRotation} className="flex items-center gap-1.5 text-sm font-semibold text-[#0E79B2] border border-[#DCD3C2] rounded-lg px-3 py-1.5 hover:border-[#0E79B2]">
            <Plus size={14} /> Plage de rotation
          </button>
          <button onClick={scheduleOps.addDiner} className="flex items-center gap-1.5 text-sm font-semibold text-[#0E79B2] border border-[#DCD3C2] rounded-lg px-3 py-1.5 hover:border-[#0E79B2]">
    ...
    <div className="border border-[#E3DACB] rounded-xl p-3 bg-white/60">
      <div className="flex items-start gap-2">
        <div className="flex flex-col gap-1 pt-1">
          <button disabled={isFirst} onClick={() => ops.move(row.id, -1)} className="text-[#B3A990] hover:text-[#0E79B2] disabled:opacity-30">
            <ChevronRight size={14} style={{ transform: "rotate(-90deg)" }} />
          </button>
          <button disabled={isLast} onClick={() => ops.move(row.id, 1)} className="text-[#B3A990] hover:text-[#0E79B2] disabled:opacity-30">
            <ChevronRight size={14} style={{ transform: "rotate(90deg)" }} />
          </button>
        </div>
...
function PrintView({ theme, dateLabel, groups, computedRows, kept, materialList, isMercredi, mercredis, activitesParMercredi, transitionEnabled, transitionData, transitionImages, onBack }) {
  ...
  const openPrintableInNewTab = () => {
    ...
    const headerCells = groups.map((g) => `<th style="text-align:left;padding:10px;background:#0E79B2;color:white;font-size:11px;text-transform:uppercase;">${escapeHtml(g)}</th>`).join("");
    ...
    if (isMercredi && mercredis.length > 0) {
      ...
      monthlyHtml = `<p style="color:#272635;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:1px;">Horaire mensuelle</p>
<h1 style="color:#272635;margin:4px 0 12px;">${escapeHtml(theme) || "Thème du mois"}</h1>
<table style="margin-bottom:24px;"><thead><tr><th style="text-align:left;padding:10px;background:#0E79B2;color:white;font-size:11px;text-transform:uppercase;">Date</th><th style="text-align:left;padding:10px;background:#0E79B2;color:white;font-size:11px;text-transform:uppercase;">Activité</th></tr></thead><tbody>${monthlyRows}</tbody></table>
<div style="page-break-before:always;"></div>`;
    ...
    const fichesHtml = kept.map((st) => {
      ...
      return `<div style="page-break-before:always;padding:24px 0;">
        <p style="color:#272635;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:1px;">${escapeHtml(st.lieu || "Plateau")}</p>
        <h2 style="color:#272635;margin:4px 0 12px;">${escapeHtml(st.nom)}</h2>
        <p style="color:#7A7362;">${escapeHtml(st.age)} · ${escapeHtml(st.duree)}</p>
        ${st.amorce ? `<h3 style="color:#0E79B2;font-size:13px;text-transform:uppercase;margin-top:16px;">Amorce</h3><p style="font-style:italic;">${escapeHtml(st.amorce)}</p>` : ""}
        ${etapes ? `<h3 style="color:#0E79B2;font-size:13px;text-transform:uppercase;margin-top:16px;">Déroulement</h3><ol style="padding-left:18px;">${etapes}</ol>` : ""}
        ${materiel ? `<h3 style="color:#0E79B2;font-size:13px;text-transform:uppercase;margin-top:16px;">Matériel</h3><ul style="list-style:none;padding-left:0;">${materiel}</ul>` : ""}
      </div>`;
    }).join("");
...
${monthlyHtml}
<p style="color:#272635;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:1px;">Horaire de la journée</p>
<h1 style="color:#272635;margin:4px 0 12px;">${escapeHtml(theme) || "Thème de la journée"}</h1>
${dateLabel ? `<p style="color:#7A7362;">${escapeHtml(dateLabel)}</p>` : ""}
<table style="margin-top:16px;"><thead><tr><th style="text-align:left;padding:10px;background:#0E79B2;color:white;font-size:11px;text-transform:uppercase;">Heure</th>${headerCells}</tr></thead><tbody>${rowsHtml}</tbody></table>
${materialList.length ? `<h2 style="color:#272635;margin-top:24px;">Matériel</h2><ul style="list-style:none;padding-left:0;">${materialHtml}</ul>` : ""}
${fichesHtml}
<p style="margin-top:24px;color:#B3A990;font-size:12px;">Ouvrez le menu de partage de votre navigateur pour imprimer ou enregistrer en PDF.</p>
  ...
  return (
    <div className="print-root max-w-4xl mx-auto px-4 py-8">
      <button onClick={onBack} className="no-print flex items-center gap-1.5 text-sm font-semibold text-[#7A7362] hover:text-[#0E79B2] mb-4">
  ...
  const openPrintableInNewTab = () => {
    const rows = jours.map((jourObj) => {
      const c = visiblePeriodes.map((periode) => {
        ...
        const labelStyle = "font-weight:700;font-size:11px;color:#0E79B2;";
        ...
        return `<td style="padding:10px;vertical-align:top;border-bottom:1px solid #EDE6D8;">
          ...
          ${cell.domaines.length ? `<div style="${labelStyle}">Aspects du développement :</div><div style="font-size:11px;color:#0E79B2;">${escapeHtml(cell.domaines.join(" · "))}</div>` : ""}
        </td>`;
      }).join("");
    ...
    const headerCells = visiblePeriodes.map((p) => `<th style="text-align:left;padding:10px;background:#0E79B2;color:white;font-size:11px;text-transform:uppercase;">${escapeHtml(p)}</th>`).join("");
    ...
    jours.forEach((jourObj) => visiblePeriodes.forEach((periode) => {
      ...
      fichesHtml.push(`<div style="page-break-before:always;padding:24px 0;">
        <p style="color:#272635;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:1px;">${escapeHtml(jourObj.name)} · ${escapeHtml(periode)}${c.local ? " · " + escapeHtml(c.local) : ""}${c.duree ? " · " + escapeHtml(c.duree) : ""}</p>
        <h2 style="color:#272635;margin:4px 0 12px;">${escapeHtml(c.activite)}</h2>
        ${c.domaines.length ? `<p style="color:#7A7362;">${c.domaines.map(escapeHtml).join(" · ")}</p>` : ""}
        ${c.amorce ? `<h3 style="color:#0E79B2;font-size:13px;text-transform:uppercase;margin-top:16px;">Amorce</h3><p style="font-style:italic;">${escapeHtml(c.amorce)}</p>` : ""}
        ${etapes ? `<h3 style="color:#0E79B2;font-size:13px;text-transform:uppercase;margin-top:16px;">Déroulement</h3><ol style="padding-left:18px;">${etapes}</ol>` : ""}
        ${materiel ? `<h3 style="color:#0E79B2;font-size:13px;text-transform:uppercase;margin-top:16px;">Matériel</h3><ul style="list-style:none;padding-left:0;">${materiel}</ul>` : ""}
...
<img src="${logoUrl}" class="print-logo" alt="PLANIF" />
<p style="color:#272635;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:1px;">Résumé de la semaine</p>
<h1 style="color:#272635;margin:4px 0 14px;">Planification hebdomadaire</h1>
<table style="margin-bottom:6px;"><tr>
  <td style="padding:4px 0;color:#272635;font-weight:700;">Groupe : <span style="font-weight:400;color:#2B2A26;">${escapeHtml(groupeNom) || "—"}</span></td>
  <td style="padding:4px 0;color:#272635;font-weight:700;">Éducateur·trice : <span style="font-weight:400;color:#2B2A26;">${escapeHtml(educatrice) || "—"}</span></td>
</tr><tr>
  <td style="padding:4px 0;color:#272635;font-weight:700;">Semaine : <span style="font-weight:400;color:#2B2A26;">${escapeHtml(semaine) || "—"}</span></td>
  <td style="padding:4px 0;color:#272635;font-weight:700;">Thème : <span style="font-weight:400;color:#2B2A26;">${escapeHtml(theme) || "—"}</span></td>
</tr></table>
<table style="margin-top:16px;"><thead><tr><th style="text-align:left;padding:10px;background:#0E79B2;color:white;font-size:11px;text-transform:uppercase;">Jour</th>${headerCells}</tr></thead><tbody>${rows}</tbody></table>
${fichesHtml.join("")}
<p style="margin-top:24px;color:#B3A990;font-size:12px;">Ouvrez le menu de partage de votre navigateur pour imprimer ou enregistrer en PDF.</p>
</body></html>`;
    ...
    <div>
      ...
      {wtab === "configurer" ? (
        <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
          ...
          <SectionCard>
            ...
            <div className="space-y-2">
              ...
              <button onClick={addJour} className="flex items-center gap-1 text-sm font-semibold text-[#0E79B2]"><Plus size={14} /> Jour</button>
            </div>
            <button
              onClick={async () => {
                setSavingLieux(true);
          ...
          <SectionCard>
            ...
            <div className="flex flex-wrap gap-2">
              {periodes.map((p, idx) => (
                <div key={idx} className="flex items-center gap-1 bg-white border border-[#DCD3C2] rounded-lg pl-1 pr-1 py-1">
                  <label className="flex items-center gap-1.5 pl-1.5 cursor-pointer">
                    <input type="checkbox" checked={selectedPeriodes.includes(p)} onChange={() => togglePeriodeSelected(p)} className="w-4 h-4 accent-[#0E79B2]" />
              ...
              <button onClick={addPeriode} className="flex items-center gap-1 text-sm font-semibold text-[#0E79B2] px-2"><Plus size={14} /> Période</button>
            </div>
                        ...
                        <div key={periode} className="border border-[#E3DACB] rounded-xl p-3 bg-white/60">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <button
                              ...
                              className="text-[#0E79B2] hover:text-[#272635] disabled:opacity-40"
                          ...
                          <button onClick={() => setExpandedCell(expandedCell === key ? null : key)} className="mt-2 text-xs font-semibold text-[#0E79B2] hover:underline">
                            ...
                            <div className="mt-2 space-y-1.5">
                              ...
                              {cell.amorce ? (
                                <textarea
                                  ...
                                  rows={2}
                                  className="w-full bg-white border border-[#DCD3C2] rounded-lg px-2.5 py-1.5 text-sm text-[#2B2A26] focus:outline-none focus:ring-2 focus:ring-[#0E79B2]"
                                />
                              ) : (
                              ...
                              <textarea
                                ...
                                rows={3}
                                placeholder="Étape 1&#10;Étape 2&#10;Étape 3"
                                className="w-full bg-white border border-[#DCD3C2] rounded-lg px-2.5 py-1.5 text-sm text-[#2B2A26] placeholder-[#B3A990] focus:outline-none focus:ring-2 focus:ring-[#0E79B2]"
                              />
                              ...
                              <div className="space-y-1">
                                ...
                                <button onClick={() => setCell(jourObj.name, periode, { materiel: [...cell.materiel, ""] })} className="flex items-center gap-1 text-xs font-semibold text-[#0E79B2]">
                                  <Plus size={12} /> Item
                                </button>
                              </div>
                            </div>
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
          <button onClick={() => setWtab("configurer")} className="no-print flex items-center gap-1.5 text-sm font-semibold text-[#7A7362] hover:text-[#0E79B2] mb-4">
            <ChevronLeft size={16} /> Retour
          </button>
          {visiblePeriodes.length > 0 && (
...
function BibliothequeView({ onBack }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      <button onClick={onBack} className="flex items-center gap-1.5 text-sm font-semibold text-[#7A7362] hover:text-[#0E79B2]">
        <ChevronLeft size={16} /> Retour
      </button>
      ...
      <div className="max-w-md">
        ...
        <div className="mt-1">
          <input
            ...
            className="w-full bg-white border border-[#DCD3C2] rounded-lg px-3 py-2 font-bold text-lg focus:outline-none focus:ring-2 focus:ring-[#0E79B2]"
            ...
            return (
              <div key={item.id} className={`bg-white border border-[#E3DACB] rounded-2xl p-4 ${isOpen ? "sm:col-span-2" : ""}`}>
                ...
                <button
                  ...
                  className="mt-2 text-xs font-semibold text-[#0E79B2] hover:underline"
                >
