...
// ---------- Matériel détecté automatiquement (cartes à découper) ----------
// Repère si "cartes" apparaît dans la liste de matériel d'une activité
// (peu importe le mot exact — "cartes alimentaires", "des cartes", etc.),
...
Propose exactement 8 mots ou courtes expressions (1 à 2 mots chacun, en français) en lien avec ce thème et cette activité, adaptés à des enfants du primaire, pour fabriquer 8 cartes à découper.

Réponds UNIQUEMENT avec un tableau JSON valide de 8 chaînes, sans texte avant/après, format exact :
...
function CartesPrintPage({ nomActivite, theme, items }) {
  ...
  return (
    <div className="print-page bg-white border border-[#E3DACB] print-shadow-off rounded-2xl p-8 mb-8" style={{ boxShadow: "0 1px 3px rgba(43,42,38,0.06)" }}>
      <p className="text-xs font-bold tracking-widest uppercase" style={{ color: COLORS.marine }}>Matériel — cartes à découper</p>
      <h2 className="text-2xl font-bold mt-1" style={{ fontFamily: "Baloo 2, sans-serif", color: COLORS.mossDark }}>{nomActivite}</h2>
      <div className="leaf-underline w-16 mt-3 mb-6" />
      <div className="grid grid-cols-4 gap-4">
        {items.slice(0, 8).map((mot, i) => (
          <div key={i} className="border border-dashed border-[#DCD3C2] rounded-xl py-6 px-2 text-center" style={{ fontSize: 12.5, fontWeight: 700, color: COLORS.mossDark }}>
            {mot}
          </div>
        ))}
      </div>
...
// ---------- Matériel détecté automatiquement (détecteur général) ----------
// Repère les items de matériel qui décrivent un document à fabriquer/imprimer
// (fiches, étiquettes, certificats, recettes illustrées, gabarits, etc.),
// sans viser un seul mot précis — pour couvrir plusieurs formulations
// possibles. Exclut volontairement ce qui est déjà couvert par les
// détecteurs spécifiques (bingo, cartes, quiz, collation) pour éviter les
// doublons.
const MATERIEL_GENERE_MOTS_CLES = [
  "fiche", "illustr", "pre-imprim", "préimprim", "etiquette", "étiquette",
  "certificat", "gabarit", "modele", "modèle", "recette", "affiche", "pictogramme",
];
function activiteNecessiteMaterielGenere(materiel) {
  const norm = (s) => String(s || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  const dejaCouvert = (s) => {
    const n = norm(s);
    return n.includes("carte") || n.includes("bingo") || n.includes("quiz") || n.includes("collation");
  };
  const matches = [];
  (materiel || []).forEach((m) => {
...
function buildMaterielGenerePrompt({ theme, nomActivite, itemMateriel }) {
  return `Tu prépares un document imprimable pour une activité de service de garde en milieu scolaire, à partir d'un item de matériel mentionné.

Activité : "${nomActivite}"
Thème de la journée : "${theme || "non précisé"}"
Item de matériel à préparer : "${itemMateriel}"

Détermine le format le plus approprié pour ce document et prépare son contenu, adapté à des enfants du primaire.
- Si c'est un document à REMPLIR par les enfants (ex. fiche d'identité, étiquette, certificat) : choisis "type": "a_remplir" et fournis une liste de champs (labels courts, sans les réponses — les enfants les remplissent à la main).
- Si c'est un document avec des ÉTAPES ou une liste ordonnée (ex. recette illustrée, marche à suivre) : choisis "type": "etapes" et fournis les étapes en texte simple.
- Sinon, pour une liste simple (ex. affiche, liste de mots) : choisis "type": "liste" et fournis les éléments.

Réponds UNIQUEMENT avec un objet JSON valide, sans texte avant/après, format exact :
{
  "titre": "Titre court du document",
  "type": "a_remplir",
  "champs": ["Nom", "Habitat", "Ce que je mange", "Mon super-pouvoir"],
  "etapes": [],
  "items": []
}
(Ne remplis que le tableau correspondant au "type" choisi — laisse les deux autres vides.)`;
}

function MaterielGenerePrintPage({ nomActivite, itemMateriel, data }) {
  if (!data || !data.titre) return null;
  return (
    <div className="print-page bg-white border border-[#E3DACB] print-shadow-off rounded-2xl p-8 mb-8" style={{ boxShadow: "0 1px 3px rgba(43,42,38,0.06)" }}>
      <p className="text-xs font-bold tracking-widest uppercase" style={{ color: COLORS.marine }}>Matériel — {itemMateriel}</p>
      <h2 className="text-2xl font-bold mt-1" style={{ fontFamily: "Baloo 2, sans-serif", color: COLORS.mossDark }}>{data.titre}</h2>
      <p className="text-[#7A7362] mt-0.5 text-sm">{nomActivite}</p>
      <div className="leaf-underline w-16 mt-3 mb-6" />

      {data.type === "a_remplir" && data.champs?.length > 0 && (
        <div className="space-y-4">
          {data.champs.map((champ, i) => (
            <div key={i}>
              <p className="text-sm font-bold text-[#7C9070] mb-1">{champ}</p>
              <div className="border-b border-[#DCD3C2]" style={{ height: 28 }} />
            </div>
          ))}
        </div>
      )}

      {data.type === "etapes" && data.etapes?.length > 0 && (
        <ol className="space-y-2">
          {data.etapes.map((etape, i) => (
            <li key={i} className="flex gap-2 text-[15px] text-[#2B2A26] leading-relaxed">
              <span className="shrink-0 font-bold" style={{ color: COLORS.marine }}>{i + 1}.</span>{etape}
            </li>
          ))}
        </ol>
      )}

      {data.type === "liste" && data.items?.length > 0 && (
        <ul className="space-y-1.5">
          {data.items.map((item, i) => (
            <li key={i} className="flex gap-2 text-[15px] text-[#2B2A26]"><span style={{ color: COLORS.marine }}>•</span>{item}</li>
          ))}
        </ul>
      )}
    </div>
...
// ---------- Matériel détecté automatiquement (quiz vrai ou faux) ----------
// Repère si "quiz" apparaît dans le nom ou le matériel d'une activité.
function activiteNecessiteQuiz(nom, materiel) {
  const norm = (s) => String(s || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  if (norm(nom).includes("quiz")) return true;
  return (materiel || []).some((m) => norm(m).includes("quiz"));
}

function buildQuizPrompt({ theme, nomActivite }) {
  return `Tu prépares un quiz vrai ou faux imprimable pour une activité de service de garde en milieu scolaire.

Activité : "${nomActivite}"
Thème de la journée : "${theme || "non précisé"}"

Propose exactement 10 affirmations (en français, une phrase courte chacune) en lien avec ce thème et cette activité, adaptées à des enfants du primaire, chacune étant soit vraie soit fausse. Varie les réponses (ni toutes vraies, ni toutes fausses).

Réponds UNIQUEMENT avec un tableau JSON valide de 10 objets, sans texte avant/après, format exact :
[{"question": "Affirmation 1", "reponse": true}, {"question": "Affirmation 2", "reponse": false}]`;
}

function QuizPrintPage({ nomActivite, theme, questions }) {
  if (!questions || questions.length < 10) return null;
  return (
    <div className="print-page bg-white border border-[#E3DACB] print-shadow-off rounded-2xl p-8 mb-8" style={{ boxShadow: "0 1px 3px rgba(43,42,38,0.06)" }}>
      <p className="text-xs font-bold tracking-widest uppercase" style={{ color: COLORS.marine }}>Matériel — quiz vrai ou faux</p>
      <h2 className="text-2xl font-bold mt-1" style={{ fontFamily: "Baloo 2, sans-serif", color: COLORS.mossDark }}>{nomActivite}</h2>
      <div className="leaf-underline w-16 mt-3 mb-6" />
      <ol className="space-y-3 mb-6">
        {questions.slice(0, 10).map((q, i) => (
          <li key={i} className="flex items-start gap-2 text-[15px] text-[#2B2A26]">
            <span className="shrink-0 font-bold" style={{ color: COLORS.marine }}>{i + 1}.</span>
            <span className="flex-1">{q.question}</span>
            <span className="shrink-0 text-xs text-[#B3A990] whitespace-nowrap">○ Vrai &nbsp; ○ Faux</span>
          </li>
        ))}
      </ol>
      <div className="pt-4 border-t border-[#EDE6D8]">
        <p className="text-xs font-bold uppercase tracking-wide mb-1.5" style={{ color: COLORS.moss }}>Corrigé</p>
        <p className="text-xs text-[#7A7362]">
          {questions.slice(0, 10).map((q, i) => `${i + 1}. ${q.reponse ? "Vrai" : "Faux"}`).join("  ·  ")}
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex justify-between pb-6">
        ...
        <button onClick={onContinue} disabled={dayType !== "libre" && kept.length === 0} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-white font-semibold disabled:opacity-40" style={{ background: COLORS.moss }}>
...
function PrintView({ theme, dateLabel, groups, computedRows, scheduleRows, kept, materialList, isMercredi, mercredis, activitesParMercredi, transitionEnabled, transitionData, transitionImages, dayType, onBack }) {
  ...
  useEffect(() => {
    kept.forEach((activite) => {
      ...
      askClaude(buildCartesPrompt({ theme, nomActivite: activite.nom }))
        ...
        .catch(() => {})
        .finally(() => { cartesEnCours.current[activite.id] = false; });
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [kept, theme]);
  ...
  useEffect(() => {
    kept.forEach((activite) => {
      ...
      askClaude(buildQuizPrompt({ theme, nomActivite: activite.nom }))
        ...
        .catch(() => {})
        .finally(() => { quizEnCours.current[activite.id] = false; });
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [kept, theme]);

  // Même logique automatique pour le détecteur général de matériel à
  ...
  useEffect(() => {
    kept.forEach((activite) => {
      const items = activiteNecessiteMaterielGenere(activite.materiel);
      items.forEach((itemMateriel) => {
          ...
          .then((data) => {
            if (data && data.titre) {
              setMaterielGenere((cur) => ({
                ...cur,
                [activite.id]: { ...(cur[activite.id] || {}), [itemMateriel]: data },
              }));
            }
          })
          .catch(() => {})
          .finally(() => { materielGenereEnCours.current[cle] = false; });
      });
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [kept, theme]);
...
<div style="page-break-before:always;page-break-inside:avoid;"></div>`;
    ...
    const fichesHtml = kept.map((st) => {
      ...
      const quizHtml = (activiteNecessiteQuiz(st.nom, st.materiel) && questionsQuiz && questionsQuiz.length >= 10) ? (() => {
        ...
        <div style="padding-top:14px;border-top:1px solid #EDE6D8;">
        <p style="color:#7C9070;font-size:11px;font-weight:700;text-transform:uppercase;margin-bottom:4px;">Corrigé</p>
        <p style="color:#7A7362;font-size:11px;">${corrige}</p>
        </div>
      </div>`;
      })() : "";
      const materielGenereHtml = activiteNecessiteMaterielGenere(st.materiel).map((itemMateriel) => {
        const data = materielGenere[st.id]?.[itemMateriel];
        if (!data || !data.titre) return "";
        let corpsHtml = "";
        if (data.type === "a_remplir" && data.champs?.length) {
          corpsHtml = data.champs.map((c) => `<div style="margin-bottom:16px;"><p style="color:#7C9070;font-weight:700;font-size:13px;margin-bottom:4px;">${escapeHtml(c)}</p><div style="border-bottom:1px solid #DCD3C2;height:24px;"></div></div>`).join("");
        } else if (data.type === "etapes" && data.etapes?.length) {
          corpsHtml = `<ol style="padding-left:18px;">${data.etapes.map((e) => `<li style="margin-bottom:6px;">${escapeHtml(e)}</li>`).join("")}</ol>`;
        } else if (data.type === "liste" && data.items?.length) {
          corpsHtml = `<ul style="list-style:none;padding-left:0;">${data.items.map((i) => `<li style="margin-bottom:4px;">• ${escapeHtml(i)}</li>`).join("")}</ul>`;
        }
        if (!corpsHtml) return "";
        return `<div style="page-break-before:always;page-break-inside:avoid;padding:24px 0;">
        <p style="color:#54634A;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:1px;">Matériel — ${escapeHtml(itemMateriel)}</p>
        <h2 style="color:#54634A;margin:4px 0 4px;">${escapeHtml(data.titre)}</h2>
        <p style="color:#7A7362;font-size:13px;margin-bottom:16px;">${escapeHtml(st.nom)}</p>
        ${corpsHtml}
      </div>`;
      }).join("");
      return `<div style="page-break-before:always;page-break-inside:avoid;padding:24px 0;">
  ...
  return (
    <div className="print-root max-w-4xl mx-auto px-4 py-8">
      ...
      {kept.map((st) => (
        ...
        {activiteNecessiteCartes(st.materiel) && (
        ...
        {activiteNecessiteMaterielGenere(st.materiel).map((itemMateriel) => (
          <MaterielGenerePrintPage
            key={itemMateriel}
            nomActivite={st.nom}
            itemMateriel={itemMateriel}
            data={materielGenere[st.id]?.[itemMateriel]}
          />
        ))}
        </React.Fragment>
      ))}
...
function WeeklyGridTool({ initialData }) {
  ...
  // Même logique automatique pour les cartes à découper (détectées via le
  // mot "cartes" dans le matériel d'une case).
  useEffect(() => {
    jours.forEach((jourObj) => {
      visiblePeriodes.forEach((periode) => {
        const cell = getCell(jourObj.name, periode);
        if (!activiteNecessiteCartes(cell.materiel)) return;
        const key = weeklyCellKey(jourObj.name, periode);
  ...
  // Même logique automatique pour le quiz vrai ou faux.
  useEffect(() => {
    jours.forEach((jourObj) => {
      visiblePeriodes.forEach((periode) => {
        const cell = getCell(jourObj.name, periode);
        if (!activiteNecessiteQuiz(cell.activite, cell.materiel)) return;
        const key = weeklyCellKey(jourObj.name, periode);
        ...
        askClaude(buildQuizPrompt({ theme, nomActivite: cell.activite }))
          ...
          .catch(() => {})
          .finally(() => { quizEnCoursWeek.current[key] = false; });
      });
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cells, theme, visiblePeriodes.join(","), jours.map((j) => j.name).join(",")]);
  ...
  // Même logique automatique pour le détecteur général de matériel à
  // fabriquer, en mode hebdomadaire.
  useEffect(() => {
    jours.forEach((jourObj) => {
      visiblePeriodes.forEach((periode) => {
        const cell = getCell(jourObj.name, periode);
        const key = weeklyCellKey(jourObj.name, periode);
        const items = activiteNecessiteMaterielGenere(cell.materiel);
        items.forEach((itemMateriel) => {
            ...
            .then((data) => {
              if (data && data.titre) {
                setMaterielGenereWeek((cur) => ({
                  ...cur,
                  [key]: { ...(cur[key] || {}), [itemMateriel]: data },
                }));
              }
            })
            .catch(() => {})
            .finally(() => { materielGenereEnCoursWeek.current[cle] = false; });
        });
      });
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const openPrintableInNewTab = () => {
    ...
    jours.forEach((jourObj) => visiblePeriodes.forEach((periode) => {
      ...
      if (activiteNecessiteQuiz(c.activite, c.materiel) && questionsQuizC && questionsQuizC.length >= 10) {
        ...
        <div style="padding-top:14px;border-top:1px solid #EDE6D8;">
        <p style="color:#7C9070;font-size:11px;font-weight:700;text-transform:uppercase;margin-bottom:4px;">Corrigé</p>
        <p style="color:#7A7362;font-size:11px;">${corrigeQuiz}</p>
        </div>
      </div>`);
      }
      const materielGenereItemsC = activiteNecessiteMaterielGenere(c.materiel);
      materielGenereItemsC.forEach((itemMateriel) => {
        const data = materielGenereWeek[weeklyCellKey(jourObj.name, periode)]?.[itemMateriel];
        if (!data || !data.titre) return;
        let corpsHtml = "";
        if (data.type === "a_remplir" && data.champs?.length) {
          corpsHtml = data.champs.map((ch) => `<div style="margin-bottom:16px;"><p style="color:#7C9070;font-weight:700;font-size:13px;margin-bottom:4px;">${escapeHtml(ch)}</p><div style="border-bottom:1px solid #DCD3C2;height:24px;"></div></div>`).join("");
        } else if (data.type === "etapes" && data.etapes?.length) {
          corpsHtml = `<ol style="padding-left:18px;">${data.etapes.map((e) => `<li style="margin-bottom:6px;">${escapeHtml(e)}</li>`).join("")}</ol>`;
        } else if (data.type === "liste" && data.items?.length) {
          corpsHtml = `<ul style="list-style:none;padding-left:0;">${data.items.map((i) => `<li style="margin-bottom:4px;">• ${escapeHtml(i)}</li>`).join("")}</ul>`;
        ...
        <p style="color:#54634A;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:1px;">Matériel — ${escapeHtml(itemMateriel)}</p>
        <h2 style="color:#54634A;margin:4px 0 4px;">${escapeHtml(data.titre)}</h2>
        <p style="color:#7A7362;font-size:13px;margin-bottom:16px;">${escapeHtml(c.activite)}</p>
        ${corpsHtml}
      </div>`);
      });
    }));
      ...
      {wtab === "configurer" ? (
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          {fiches.map(({ key, jour, periode, cell }) => (
            ...
            {activiteNecessiteCartes(cell.materiel) && (
            ...
            {activiteNecessiteMaterielGenere(cell.materiel).map((itemMateriel) => (
              <MaterielGenerePrintPage
                key={itemMateriel}
                nomActivite={cell.activite}
                itemMateriel={itemMateriel}
                data={materielGenereWeek[key]?.[itemMateriel]}
              />
            ))}
            </React.Fragment>
          ))}
